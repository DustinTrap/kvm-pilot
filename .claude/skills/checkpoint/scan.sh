...
PROTECTED="main"
SOT_DOCS=(README.md CHANGELOG.md src/kvm_pilot/mcp/README.md src/kvm_pilot/skill/SKILL.md src/kvm_pilot/skill/references/*.md)
...
grep -rniE "\[(ws|totp)\]|(needs|requires) the .?(ws|totp).? extra|the .(ws|totp). extra" \
  README.md docs/*.md src/kvm_pilot/mcp/README.md src/kvm_pilot/skill/SKILL.md src/kvm_pilot/skill/references/*.md 2>/dev/null \
...
for c in $CLI_CMDS; do
  grep -qwF -- "$c" docs/cli.md 2>/dev/null || grep -qwFr -- "$c" src/kvm_pilot/skill/ 2>/dev/null || UNDOC="$UNDOC $c"
done
