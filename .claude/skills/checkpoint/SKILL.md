...
- **Resume pointer** = `RESUME.md` (**LOCAL-ONLY, untracked/gitignored** since #209). `RESUME.md: MISSING` → ⚠️ create it (locally — never commit it). `RESUME_LIKELY_STALE` (commits landed after the file was last *written* — mtime-based now, not git-based) → ⚠️ refresh it. `RESUME_TRACKED` → ⚠️ it re-entered git; `git rm --cached RESUME.md`. `RESUME_STAMP_STALE` → ⚠️ its internal `**Last updated:** … @ <sha>` stamp trails HEAD; refresh + re-stamp. The full verdict is § Mandatory handoff: is `RESUME.md` current vs. what actually happened this session? *(v0.3.0: pointer moved from agent memory to in-repo `RESUME.md`. v0.6.0: stamp-lag check. v0.9.0: local-only — see version history.)*
...
- `CHANGELOG_UNRELEASED_MISSING: N src/kvm_pilot *.py file(s) changed since CHANGELOG.md was last edited (<sha>) but no [Unreleased] section` → ⚠️ code shipped to `main` after the CHANGELOG was last touched, with no `[Unreleased]` section to record it. Add a `## [Unreleased]` section describing it so the next release doesn't forget it. Fires the **merge-without-release** case; anchored on the last CHANGELOG edit (not a tag), so it stays quiet right after a release when the release commit dated a section for that same code. *(v0.7.0; anchor fixed v0.8.0)*
...
- **A new CLI subcommand can under-document the shipped surface** even when no doc "lies." `CLI_CMDS_UNDOCUMENTED` lists argparse subcommands (`cli.py` `add_parser`) absent from **both** `docs/cli.md` and the bundled `SKILL.md` command set. A command that landed *before* the latest tag but *after* a doc's last edit slips through the changed-since-tag filter — anything listed is ⚠️: add it to the cli.md command table **and** the SKILL.md command list. *(v0.5.0)*
...
A checkpoint that ends with no fresh handoff **has not finished**. The resume pointer for this project is **`RESUME.md` at the repo root — a LOCAL-ONLY, untracked (gitignored) file** *(v0.3.0 made it git-tracked at the operator's request; v0.9.0 reversed that per #209 — the committed file was publishing internal fleet IPs/usernames/key-install state in a public repo. The durable-pointer property was consciously traded for opsec: the resume now survives only on this machine; cross-machine state lives in the agent memory and GitHub issues)*. Before signing off:
...
3. **Save `RESUME.md` locally — do NOT commit it** (#209: it may name fleet IPs/usernames; it is gitignored and the scan flags `RESUME_TRACKED` if it sneaks back into git). Anything a *fresh clone or another agent* must know goes in a GitHub issue or the agent memory instead, scrubbed of fleet details.
...
An all-✅ scan with a stale or missing `RESUME.md` is an **incomplete checkpoint** — refresh it (locally) regardless.
...
| 0.5.0 | **C4 now flags a shipped CLI subcommand missing from the docs.** Added a `CLI_CMDS_UNDOCUMENTED` scan line — argparse subcommands (`cli.py` `add_parser`) diffed against the command names present in `docs/cli.md` + `SKILL.md` — plus a C4 rubric bullet. | a14 shipped `route`/`host-exec` (added *before* the a14 tag, *after* `cli.md`'s last edit), so both the CLI reference and the bundled skill's command set silently omitted them; the changed-since-tag docs filter passed and it was caught only by hand. |
| 0.6.0 | **C2 now catches a RESUME body updated without bumping its own stamp.** Added a `RESUME_STAMP_STALE` scan line — parse the `**Last updated:** … @ <sha>` stamp and flag when it trails RESUME.md's *own last commit* by ≥2 (past the normal self-commit +1) — plus a C2 rubric bullet. | This run: a `docs: #6 reboot` commit updated RESUME.md's device-state body but left the `Last updated` stamp pointing 2 commits back (`9708ea7`); the file was committed *at* HEAD so `RESUME_LIKELY_STALE` stayed silent, and the stale stamp was caught only by hand-reading line 7. |
| 0.7.0 | **C4 now catches feature work merged to `main` with no `[Unreleased]` CHANGELOG section.** The old check only ran when the `[Unreleased]` section already existed, so it fired around a *release* but stayed silent for the common **merge-without-release** case. Added a `CHANGELOG_UNRELEASED_MISSING` scan line (src/docs commit count since the latest tag when no `[Unreleased]` section exists) + a C4 rubric bullet. | This run: the IPMI driver (#206) + ipmi_sim cross-check (#207) merged to `main` post-`v0.1.0b6` with no `[Unreleased]` section; `CHANGELOG_HAS_0.1.0b6: yes` so no existing check fired, and the omission was caught only by hand. |
| 0.9.0 | **RESUME.md is local-only again — untracked and gitignored (reverses v0.3.0).** C2 + Mandatory handoff retargeted: never commit `RESUME.md`; staleness judged by file *mtime* vs commits (`RESUME_LIKELY_STALE`), a new `RESUME_TRACKED` flag catches it re-entering git, and the stamp-lag check now compares against HEAD. Cross-machine handoff state goes to GitHub issues / agent memory, scrubbed. | The 2026-07-14 docs audit (#209) found the committed `RESUME.md` publishing internal fleet LAN IPs, usernames, and SSH-key-install status in the public repo — the durable-pointer property v0.3.0 bought was leaking opsec state. Operator chose local-only + gitignore (history left as-is, RFC1918 accepted). |
| 0.8.0 | **Re-anchored `CHANGELOG_UNRELEASED_MISSING` on the last CHANGELOG edit, not the local tag.** Now counts `src/kvm_pilot/*.py` files changed since the commit that last touched `CHANGELOG.md` (was: src/docs commits since `LOCAL_TAG`). | Immediate false positive on the very next (post-release) run: the local tag `v0.1.0b6` trailed the just-published `v0.1.0b7`, so the tag-window counted the 7 b7 commits — already recorded in the dated `[0.1.0b7]` section — as "unrecorded," reading as 1 item needing attention when nothing did. The v0.7.0 real case (code lands, CHANGELOG untouched) still fires; the just-released case (release commit dated the section for that code → 0 .py since that commit) now stays quiet. |
| 0.10.0 | **C1 now scans for committed secret literals (`SECRET_SCAN`).** Greps the working diff + last 25 commits' added lines for quoted `password=/secret=/token=` literals, inline `IPMI_PASSWORD=<value>`, and private-key headers; excludes env/placeholder refs and stock defaults (`calvin`/`admin`/…). A hit is a 🔴 "inspect + ROTATE if real." | Same run as v0.9.0, one step sharper: #209 caught RESUME.md leaking IPs/usernames, but nothing caught that this session also committed a real **plaintext iDRAC password** (`IPMI_PASSWORD=moody`) to RESUME.md and pushed it to the public repo — found only by a hand `grep` after the #209 policy change. A password needs rotation, not just RFC1918 acceptance; the scan now surfaces that class belatedly so it can't sit unnoticed. |
| 0.11.0 | **`SECRET_SCAN` now excludes the AMT RFB test fixtures** (`"rfbpass"`/`"rfb-only"`/`Abcd123`/`"secret"`) alongside the existing `calvin`/`admin` stock-default excludes. | The AMT-driver session (b9) added ~10 `kvm_password="rfbpass"` / `amt_kvm_password="rfb-only"` / `kvm_password="Abcd123!"` **test-constructor** literals; the scan flagged all 10 as possible real secrets, and each had to be hand-triaged as a fixture. The real RFB password lives only in the gitignored config (never committed), so these literals are always fixtures — noise that buried the real-leak signal the scan exists to surface. |
| 0.13.0 | **C1 no longer reports a pushed branch as unpushed, and C4 guards the mcp/README's dependency claim.** `scan.sh` falls back to `origin/<branch>` when no tracking ref is configured, so the ahead/behind counts (the real work-at-risk signal) still run; and a new `test_mcp_readme_declares_the_real_mcp_dependency` guard ties that README's stated SDK range to pyproject. | This run (b12 release): `main` was fully pushed (HEAD == origin/main) but `git push origin main` sets no tracking ref, so the scan printed `UPSTREAM: (none — branch not pushed)` and **suppressed the ahead/behind lines entirely** — the surface whose whole job is "is my work backed up" went silent on a released session. Separately, this session's floor change (`mcp>=1.10` → `>=1.17`) made `src/kvm_pilot/mcp/README.md` state a version range that no longer existed — the v0.4.0 "code change makes an unchanged doc lie" case, caught by hand again, so it is now a test rather than a rubric reminder. |
| 0.12.0 | **`SECRET_SCAN` now excludes AMT RFB password *doc illustrations*** by their `EXACTLY 8 chars` / `EXAMPLE` / `set your own` markers (added to `SECRET_EXCL`, matched at the doc's literal casing since the exclusion is case-sensitive). | The b10 `docs/amt-onboarding.md` added two new 8-char RFB password EXAMPLES (`amt_kvm_password = "Chang3M!"` / `"Abcd1@ef"`); the v0.11.0 exclusion was keyed to the *values* (`rfbpass`/`Abcd123`), so new example values slipped through and both flagged as possible real secrets on a code-only session that never touched AMT. Excluding by the tutorial marker (which a real leaked credential wouldn't carry) covers future 8-char RFB examples too. Also noted: the lowercase `example` already in the list never matched the doc's uppercase `EXAMPLE` — the grep is `-I` (ignore-binary), not `-i` (ignore-case). |
