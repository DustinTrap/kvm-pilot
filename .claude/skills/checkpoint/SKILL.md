...
- **Resume pointer** = `RESUME.md` (git-tracked). `RESUME_MISSING` → ⚠️ create it. `RESUME_LIKELY_STALE` (commits landed after its last update) or `RESUME_UNCOMMITTED` → ⚠️ refresh + commit + push it. `RESUME_STAMP_STALE` → ⚠️ the file's body was updated in a later commit but its internal `**Last updated:** … @ <sha>` stamp wasn't bumped (so it reads "fresh" to the commit-based checks yet points a reader at an old SHA) → **re-stamp the `Last updated` line to HEAD**, commit, push. The full verdict is § Mandatory handoff: is `RESUME.md` current vs. what actually happened this session? *(v0.3.0: pointer moved from agent memory to in-repo `RESUME.md`. v0.6.0: added the stamp-lag check.)*
