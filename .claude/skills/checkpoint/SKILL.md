...
- `CHANGELOG_UNRELEASED_MISSING: N src/docs commit(s) since <tag> but no [Unreleased] section` → ⚠️ the CHANGELOG has *no* `[Unreleased]` section yet feature/doc work has merged to `main` since the last release. Add a `## [Unreleased]` section capturing it so the next release doesn't forget it. This fires the common case the release-event checks miss: a session that **merges without releasing**. *(v0.7.0)*
