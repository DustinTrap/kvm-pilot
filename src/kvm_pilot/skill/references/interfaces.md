...
| List what the driver supports | **MCP** `capabilities` or CLI `capabilities` | Structural/offline — no network, no preflight. Use it to pick the right interface up front. It also reports **`video_scope`**, which `video` alone can't answer: `firmware` sees BIOS/POST/GRUB (AMT, BMC KVM), `capture` sees only what the host routes to an external output — **blind below the OS on laptops** — and `none` has no video at all. Check it *before* planning firmware/boot work (#210). |
...
| Mount / eject install media | **MCP** `mount_iso` / `eject`, or CLI `mount` / `eject` | Virtual media (ISO path or URL). MCP needs `KVM_PILOT_MCP_ALLOW_MEDIA` + approval. **AMT (IDE-R) streams the disc from the client process**: via MCP the server holds it across calls (`mount_iso` → `set_boot_device('cd')`, which reports `use_ider: true` → `power reset`, **no eject in between**); the CLI `mount` serves in the foreground. Reset **once**; if the host boots its disk anyway, confirm `list_virtual_media` shows `connected: true` and stop after two tries — session churn + resets wedge the ME to a G3-only recovery (#251/#252). |
