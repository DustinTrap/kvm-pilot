...
def file_firmware_report(
    ...
    *,
    ...
    The emission side of the registry telemetry loop, shared by the CLI
    ``firmware-check`` (default-on) and the MCP ``file_firmware_report`` tool
    (#190, behind the EXTERNAL_WRITE gate). Never silent and never fatal — the
    returned outcome dict says what happened and why, and a filing failure must
    not break the caller. Shells out to the ``gh`` CLI (it owns auth); a missing
    ``gh`` is a graceful ``filed=False`` reason, not an exception.
    """
    import datetime
    import shutil
    ...
    if source:
        sub["source"] = source
    sub["date"] = date or datetime.date.today().isoformat()
    if not sub.get("source"):
        return {"filed": False, "reason": "no known source URL for this device — pass a "
                                          "release-channel URL (--source on the CLI, source= "
                                          "on the MCP tool) to file the report"}
