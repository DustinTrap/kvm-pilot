...
| `virtual_media` | `mount`, `eject`, `media-list` · MCP `mount_iso`, `eject`, `list_virtual_media` | conditional | live:dell Latitude 5411@AMT 14.1.79 (one in-OS read + one iPXE boot, #213 — not yet a ledger row) + emulator | **IDE-R**: the ISO is streamed **from the client process** as a virtual ATAPI CD (#213). The session must outlive the reset that boots from it, so it is process-wide: `close()` leaves it attached, the next driver for the host adopts it, `eject` detaches; the CLI `mount` **serves in the foreground** until Ctrl-C (#252). `boot-device cd` sets `UseIDER`/`IDERBootDevice=1` while a disc is served (`use_ider: true` in the result) — the plain CD/DVD source means a physical drive. CD images only; USB-R not implemented. |
