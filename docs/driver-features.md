...
| **live:`<vendor> <product>@<fw>`** | Confirmed on real hardware, with a row in `test_runs.jsonl`. Annotated with the **derived maturity** (below) where the ledger provides one, e.g. `live:gl.inet RM1PE@V1.9.1 (ga)`. |
...
|---|---|---|---|---|
| `system_info` | `info` · MCP `info` | conditional | live:gl.inet RM1PE@V1.5.1 (rc), @V1.9.1 (ga) | Fields present and correct, but the raw identity **lies** (rpi/rpi4 on RV1126); trust `get_firmware_info` (reads `/api/upgrade/version`). Dual version numbers (GL product vs kvmd component). |
| `power` | `power`, `power-cycle` · MCP `power`, `power_state` | **false-report** | read: live, untrusted — @V1.9.1 (ga), @V1.5.1 (rc); actuate: never (ATX unwired) | ATX **always reports off / enabled=false / LEDs false** on RM1PE even when running — `is_powered_on()` and the *result* of on/off/cycle **cannot be confirmed via ATX**. Verify visually (snapshot/vision). On unwired ATX, actuation now fails with a **clear `CapabilityError`** ("power control unavailable — see `kvm-pilot paths`") instead of an opaque `HTTP 500` (#174). Actuation itself has **no ledger run** (`never_exercised: power` — no OOB power on the fleet). |
...
| `logs` | `logs` · MCP `logs` | **reliable** | live:gl.inet RM1PE@V1.5.1 (rc), @V1.9.1 (ga) | kvmd `/api/log`; `seek` = seconds of lookback. Verified on both firmwares — it's how the RV1126 encoder wedge (venc/vpss/vvi in D-state) was diagnosed. `follow`/tail is refused (blocking transport). |
...
Structural set: `system_info, power, virtual_media, boot_config, logs, boot_progress, sensors`.
...
| `boot_config` | `boot-device` · MCP `boot_options`, `set_boot_device` | unverified | emulator (sushy-tools) | `BootSourceOverride` PATCH on `ComputerSystem.Boot`: `Once` (next-boot) / `Continuous` (persistent) / `Disabled` (clear). `BootSourceOverrideMode` (UEFI/Legacy) is set only where the target advertises it — older iLO4/iDRAC7 omit it. Gated `redfish.set_boot_device` ([#28](https://github.com/DustinTrap/kvm-pilot/issues/28)/#201); vendor quirks ladder in the [runbook](hardware-test-plan-ilo-idrac.md) Phase B. |
...
one unit, so `beta` shows as `n=1` on the wiki (≥3 runs promote further). See
...
| Capability | CLI / MCP surface | Reliability | Testing level | Notes |
|---|---|---|---|---|
| `power` | `power`, `power-cycle` · MCP `power`, `power_state` | reliable | live:dell Latitude 5411@AMT 14.1.67 (beta) + emulator | CIM `RequestPowerStateChange` (on `2` / soft-off `8` / hard-off `6` / reset `10`); reads `CIM_AssociatedPowerManagementService.PowerState`. Read + reset exercised live (no false-report mode, unlike GL ATX). |
| `system_info` | `info` · MCP `info` | reliable | live:dell Latitude 5411@AMT 14.1.67 (beta) + emulator | `CIM_Chassis`/package identity + `AMT_SetupAndConfigurationService` (AMT version, provisioning state); best-effort — one faulting field never blanks the rest. |
| `boot_config` | `boot-device` | conditional | live:dell Latitude 5411@AMT 14.1.67 (beta) + emulator | **Single-use only**: `pxe`/`cd`/`hdd`/`none` via `ChangeBootOrder`+`SetBootConfigRole` (verified live). The source override is **write-only** — `get_boot_options` reports `override_readable: false`, so confirm by watching the boot. **`bios` (BIOSSetup boot-to-setup) is firmware-dependent** — rejected on this unit with a clear `CapabilityError` (#215). `--persistent`, `usb`/`diag` rejected. |
| `serial_console` | `console` · `serial_read`/`serial_write` | conditional | live:dell Latitude 5411@AMT 14.1.67 (beta, channel) + emulator | SOL over 16994 via `amtterm` (password via `AMT_PASSWORD` env, never argv). **Channel validated live** (clean CONNECT, session alive, bidirectional `serial_write`); no BIOS/OS serial *text* on the Latitude — a laptop BIOS with no serial console redirection (unlike a server BIOS). Content is target-limited. |
| `video` | `snapshot` · MCP `snapshot` | conditional | live:dell Latitude 5411@AMT 14.1.67 (beta, 1080p BIOS) + emulator | AMT **RFB 4.0**: reply `RFB 003.008`, native 16-bpp RGB565 (no `SetPixelFormat`), **RLE(16)/ZRLE** over standard zlib + RAW + DesktopSize → PNG. Captures graphical BIOS/POST/GRUB; **not** legacy VGA text mode (resets after the framebuffer request). Single-session (SAP-cycle retry). |
| `hid` | `type`, `press`, `key`, `mouse-*` · MCP `type`, `press_key` | unverified | emulator (RFB) | KeyEvent/PointerEvent over the RFB session; X11 keysym map (pixel-native, maps onto the framebuffer). Keys/clicks gated; mouse *moves* ungated. |
| `virtual_media` | `mount`, `eject`, `media-list` · MCP `mount_iso`, `eject`, `list_virtual_media` | conditional | **streaming:** live:dell Latitude 5411@AMT 14.1.79 (one in-OS read + one iPXE boot, #213 — not yet a ledger row). **Session lifetime + `UseIDER` boot config (#252): emulator only — UNVERIFIED on hardware**; see the [support matrix](https://github.com/DustinTrap/kvm-pilot/wiki/Hardware-Compatibility) | **IDE-R**: the ISO is streamed **from the client process** as a virtual ATAPI CD (#213). The session must outlive the reset that boots from it, so it is process-wide: `close()` leaves it attached, the next driver for the host adopts it, `eject` detaches; the CLI `mount` **serves in the foreground** until Ctrl-C (#252). `boot-device cd` sets `UseIDER`/`IDERBootDevice=1` while a disc is served (`use_ider: true` in the result) — the plain CD/DVD source means a physical drive. CD images only; USB-R not implemented. |
