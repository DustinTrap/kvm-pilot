...
driver = "glkvm"            # auto (default: probe the device, #235) | pikvm | glkvm |
                            # blikvm | redfish | ipmi | amt | ssh | fake
                            # "ssh" = OS plane: a host with no KVM/BMC at
                            # all, reached over its own SSH (#248).
                            # Explicit only — auto never selects it.
...
| `driver` | `KVM_PILOT_DRIVER` | `auto` | `auto` (probe the device, #235 — see [Driver auto-detection](architecture.md#driver-auto-detection-auto-235)) \| `pikvm` \| `glkvm` \| `blikvm` \| `redfish` \| `ipmi` \| `amt` \| `ssh` \| `fake`; the CLI `--driver` flag overrides. `auto` never selects `ssh` — the OS plane is explicit-only (#248), and `ssh` needs `ssh_host` (the target's own address) since `host` means "the appliance" everywhere else. |
